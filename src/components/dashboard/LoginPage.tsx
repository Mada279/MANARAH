...
export default function LoginPage() {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden ${theme === 'dark' ? 'bg-[#071020]' : 'bg-[#F7F1E5]'}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          <div className="flex justify-center mb-5"><LanguageThemeControls /></div>
          ...
          <h1 className="text-3xl font-black text-gold-gradient font-arabic mb-2">{t('login.title')}</h1>
        ...
        <form onSubmit={handleSubmit} className="gradient-border p-6 md:p-8 space-y-5">
          <div>
            <label className="block text-sm text-gray-400 mb-2 font-cairo">{t('login.email')}</label>
            <input
              type="email"
              value={email}
          ...
          <div className="flex items-center justify-between text-xs text-gray-600 font-cairo">
            ...
            <a href="#hero" className="text-amber-400 hover:text-amber-300">{t('login.back')}</a>
          </div>
        </form>
      </div>
    </div>
